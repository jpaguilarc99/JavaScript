...
link.imagen = new Image();
link.imagen.src = link.url;
link.imagen.addEventListener("load", cargarCerdo);
...
function dibujar()//aqui se evalua cual imagen ha cargado, para que siga el orden de carga que queremos
{
	...
	if(link.cargaOK)
	{
		...
		link.x = x;
		link.y = y;
		papel.drawImage(link.imagen , x , y);
...
document.addEventListener("keydown", moverTecla);//identificamos el evento del teclado y restamos o sumamos posicion, segun la tecla
	function moverTecla(evento)
	{
			...
			switch (evento.keyCode)
		{
			case teclas.UP:
				dibujarOtraVez();
				moverCerdo(link.x,link.y);
				link.y = link.y - movimiento;
			break;
			case teclas.DOWN:
				dibujarOtraVez();
				moverCerdo(link.x,link.y);
